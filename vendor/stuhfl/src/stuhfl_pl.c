...
#define TRACE_PL_LOG_CLEAR()    { STUHFL_F_LogClear(STUHFL_D_LOG_LEVEL_TRACE_PL); }
#define TRACE_PL_LOG_APPEND(...){ STUHFL_F_LogAppend(STUHFL_D_LOG_LEVEL_TRACE_PL, __VA_ARGS__); }
#define TRACE_PL_LOG_FLUSH()    { STUHFL_F_LogFlush(STUHFL_D_LOG_LEVEL_TRACE_PL); }

#define TRACE_PL_LOG_START()    { STUHFL_F_LogClear(STUHFL_D_LOG_LEVEL_TRACE_PL); }
#define TRACE_PL_LOG(...)       { STUHFL_F_LogAppend(STUHFL_D_LOG_LEVEL_TRACE_PL, __VA_ARGS__); STUHFL_F_LogFlush(STUHFL_D_LOG_LEVEL_TRACE_PL); }
...
{
    STUHFL_T_RET_CODE ret = STUHFL_ERR_NONE;


    STUHFL_T_ParamTypeConnectionPort port = NULL;
    uint32_t br;
    ret = STUHFL_F_GetParam(STUHFL_PARAM_TYPE_CONNECTION | STUHFL_KEY_PORT, &port);
    ret |= STUHFL_F_GetParam(STUHFL_PARAM_TYPE_CONNECTION | STUHFL_KEY_BR, &br);
...
{
    return &snd[STUHFL_COMM_PAYLOAD_POS];
}

// --------------------------------------------------------------------------
uint16_t STUHFL_F_Get_RcvCmd()
{
    return STUHFL_COMM_GET_CMD(rcv);
}

// --------------------------------------------------------------------------
STUHFL_T_RET_CODE STUHFL_F_Get_RcvStatus()
{
    return (STUHFL_T_RET_CODE)(int16_t)STUHFL_COMM_GET_STATUS(rcv);    // Keep status sign
}

// --------------------------------------------------------------------------
uint8_t *STUHFL_F_Get_RcvPayloadPtr()
{
    return &rcv[STUHFL_COMM_PAYLOAD_POS];
}
...
STUHFL_T_RET_CODE STUHFL_F_Rcv_Dispatcher(STUHFL_T_DEVICE_CTX device, uint8_t *payloadData, uint16_t *payloadDataLen)
{
    ...
    *payloadDataLen = 0;
    ...
    //
    uint16_t rcvLen = STUHFL_COMM_PAYLOAD_POS;

    TRACE_PL_LOG_START();
    STUHFL_T_RET_CODE ret = STUHFL_F_PlatformRcvRaw(device, rcv, &rcvLen);
    if ((ret != STUHFL_ERR_NONE) || (rcvLen != STUHFL_COMM_PAYLOAD_POS)) {
        TRACE_PL_LOG("Rx <<< (%04d) .. no data packet received", rcvLen);
        return STUHFL_ERR_TIMEOUT;
    }
    *payloadDataLen = 0;

    // Check cmd is a valid one
    uint16_t cmd = STUHFL_COMM_GET_CMD(rcv);
    if (((cmd >> 8) > STUHFL_CG_LAST_VALID) || ((cmd & 0xFF) > STUHFL_CC_LAST_VALID)) {
        TRACE_PL_LOG("Rx <<< (%04d) .. unknown command (%04x), ignoring data ...", STUHFL_COMM_GET_PAYLOAD_LENGTH(rcv), cmd);
        return STUHFL_ERR_IO;
    }

    // 2) Payload
    expectedLen = STUHFL_COMM_GET_PAYLOAD_LENGTH(rcv);
    if (expectedLen > gRcvMaxLen-STUHFL_COMM_PAYLOAD_POS) {
        // Clamp to max size of Rx buffer
        expectedLen = (uint16_t)(gRcvMaxLen - STUHFL_COMM_PAYLOAD_POS);
    }
    uint16_t len = expectedLen;
    ret = STUHFL_F_PlatformRcvRaw(device, &rcv[STUHFL_COMM_PAYLOAD_POS], &len);
#define TB_SIZE    1024
    char tb[TB_SIZE];
    if ((ret != STUHFL_ERR_NONE) || (len < expectedLen)) {
        TRACE_PL_LOG("Rx <<< (%04d) .. receive length mismatch error (%d), ignoring RX data: 0x%s", (STUHFL_COMM_PAYLOAD_POS+len), expectedLen, byteArray2HexString(tb, TB_SIZE, rcv, (uint16_t)(STUHFL_COMM_PAYLOAD_POS+len)));
        return STUHFL_ERR_IO;
    }
    TRACE_PL_LOG("Rx <<< (%04d) 0x%s", (STUHFL_COMM_PAYLOAD_POS+len), byteArray2HexString(tb, TB_SIZE, rcv, (uint16_t)(STUHFL_COMM_PAYLOAD_POS+len)));
#undef TB_SIZE

    rcvLen = (uint16_t)(rcvLen + expectedLen);
    *payloadDataLen = expectedLen;

    if (&rcv[STUHFL_COMM_PAYLOAD_POS] != payloadData) {
        memcpy(payloadData, &rcv[STUHFL_COMM_PAYLOAD_POS], expectedLen);
    }


    // 3) Verify signature
    uint16_t mode = STUHFL_COMM_GET_PREAMBLE_MODE(rcv);
...
STUHFL_T_RET_CODE STUHFL_F_SetParam_Dispatcher(STUHFL_T_DEVICE_CTX device, STUHFL_T_PARAM param, STUHFL_T_PARAM_VALUE value)
{
    ...
    STUHFL_T_PARAM type = param & STUHFL_D_TYPE_MASK;
    param &= STUHFL_D_KEY_MASK;
...
{
    // 1. build header
    STUHFL_COMM_SET_PREAMBLE_MODE(sndData, mode);
    STUHFL_COMM_SET_PREAMBLE_ID(sndData, id);
    STUHFL_COMM_SET_STATUS(sndData, status);

    // 2. CMD + PayloadLen
    STUHFL_COMM_SET_CMD(sndData, cmd);
    payloadDataLen = (uint16_t)(payloadDataLen + ((mode & SIGNATURE_XOR_BCC) ? 1U : 0U));
    STUHFL_COMM_SET_PAYLOAD_LENGTH(sndData, payloadDataLen);

    // set sndDataLen to at least HeaderLen + CmdLen + PayloadLen
    if (payloadDataLen > gSndMaxLen-STUHFL_COMM_PAYLOAD_POS) {
        // Clamp to max size of Tx buffer
        payloadDataLen = (uint16_t)(gSndMaxLen - STUHFL_COMM_PAYLOAD_POS);
    }
    *sndDataLen = (uint16_t)(STUHFL_COMM_PAYLOAD_POS + payloadDataLen);

    // 3. payload (copy if needed)
    if (&sndData[STUHFL_COMM_PAYLOAD_POS] != payloadData) {
        memcpy(&sndData[STUHFL_COMM_PAYLOAD_POS], payloadData, payloadDataLen);
    ...
    if (mode & SIGNATURE_XOR_BCC) {
        sndData[STUHFL_COMM_PAYLOAD_POS + payloadDataLen] = 0;
        for (int i = 0; i < payloadDataLen; i++) {
            sndData[STUHFL_COMM_PAYLOAD_POS + payloadDataLen] ^= payloadData[i];
        }
