...
static STUHFL_T_LogOption gLogOptions = { 0 };
static STUHFL_T_LogData gLogData[STUHFL_D_LOG_LEVEL_COUNT] = { 0 };
...
{
    memcpy(&gLogOptions, &option, sizeof(STUHFL_T_LogOption));
    // assign buffers
    for (int i = 0; i < STUHFL_D_LOG_LEVEL_COUNT; i++) {
...
{
    if      (level & STUHFL_D_LOG_LEVEL_INFO)                 { return "I"; }
    else if (level & STUHFL_D_LOG_LEVEL_WARNING)              { return "W"; }
    else if (level & STUHFL_D_LOG_LEVEL_DEBUG)                { return "D"; }
    else if (level & STUHFL_D_LOG_LEVEL_ERROR)                { return "E"; }
    else if (level & STUHFL_D_LOG_LEVEL_TRACE_AL)             { return "AL"; }
    else if (level & STUHFL_D_LOG_LEVEL_TRACE_SL)             { return "SL"; }
    else if (level & STUHFL_D_LOG_LEVEL_TRACE_DL)             { return "DL"; }
    else if (level & STUHFL_D_LOG_LEVEL_TRACE_PL)             { return "PL"; }
    else if (level & STUHFL_D_LOG_LEVEL_TRACE_EVAL_API)       { return "EA"; }
    else { return ""; }
}

STUHFL_DLL_API uint8_t CALL_CONV STUHFL_F_LogLevel2Idx(uint32_t level)
{
    if      (level & STUHFL_D_LOG_LEVEL_INFO)            { return 0; }
    else if (level & STUHFL_D_LOG_LEVEL_WARNING)         { return 1; }
    else if (level & STUHFL_D_LOG_LEVEL_DEBUG)           { return 2; }
    else if (level & STUHFL_D_LOG_LEVEL_ERROR)           { return 3; }
    else if (level & STUHFL_D_LOG_LEVEL_TRACE_AL)        { return 4; }
    else if (level & STUHFL_D_LOG_LEVEL_TRACE_SL)        { return 5; }
    else if (level & STUHFL_D_LOG_LEVEL_TRACE_DL)        { return 6; }
    else if (level & STUHFL_D_LOG_LEVEL_TRACE_PL)        { return 7; }
    else if (level & STUHFL_D_LOG_LEVEL_TRACE_EVAL_API)  { return 8; }
    else { return 0; }
}
