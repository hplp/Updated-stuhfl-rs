...
#define STUHFL_D_RSSI_MODE_QUIET                        0x02
...
// --------------------------------------------------------------------------
typedef struct {
    uint32_t                            slotId;                         /**< O Param: Tag detection slot ID */
    uint32_t                            timestamp;                      /**< O Param: Tag detection time stamp in ms after inventoryRunnerStart. */
...
#define STUHFL_O_INVENTORY_TAG_INIT()        {  0, 0, \
...
typedef struct {
    ...
    uint32_t                            slotCnt;                        /**< O Param: Number of executed slots. slotCnt = slot with TAG + empty slot + slot with any other issue */
    ...
    uint32_t                            resendAckCnt;                   /**< O Param: Number of ACK resend due to error during EPC reception. */
    uint32_t                            noiseSuspicionCnt;              /**< O Param: Number of noise suspicion events. Event is triggered when ACK is resend and no response is received */
...
#define STUHFL_D_EVENT_NOISE_SUSPICION  0x0800
#define STUHFL_D_EVENT_TARGET_B   		0x1000

#if defined(USE_INVENTORY_EXT)
typedef struct {
...
#if defined(USE_SLOT_INFO_RFU)
    uint32_t                            rfu[4];                         /**< O Param: DBG */
#endif
...
typedef struct {
    ...
    uint8_t                             pwd[STUHFL_D_PASSWORD_LEN];   /**< I Param: Access password. If pwd set to 0x00000000 access command prior to read is skipped */
...
typedef struct {
    ...
    uint8_t                             pwd[STUHFL_D_PASSWORD_LEN];   /**< I Param: Access password. If pwd set to 0x00000000 access command prior to write is skipped. */
...
typedef struct {
    uint32_t                            wordPtr;                    /**< I Param: Word address to which write data. */
    uint8_t                             pwd[STUHFL_D_PASSWORD_LEN];           /**< I Param: Access password. If pwd set to 0x00000000 access command prior to block write is skipped. */
    uint8_t                             memoryBank;                 /**< I Param: Bank (File, EPC, TID, USER) to which write data. */
...
typedef struct {
    uint8_t                             pwd[STUHFL_D_PASSWORD_LEN];   /**< I Param: Access password. If pwd set to 0x00000000 access command prior to kill is skipped. */
    uint8_t                             killPwd[STUHFL_D_PASSWORD_LEN];     /**< I Param: Kill password. */
    uint8_t                             recommission;                       /**< I Param: Recommissionning. */
    uint8_t                             tagReply;                           /**< O Param: Tag reply. */
} STUHFL_T_Kill;
#define STUHFL_O_KILL_INIT()             {{0}, {0}, 0, 0}
